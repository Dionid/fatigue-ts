...
import { RequestTimeoutError } from '../../typed-errors/src'
...
export const LQ = {
  new: <Args extends any[], Result>(fn: (...args: Args) => Promise<Result>, options: { timeout?: number } = {}) => {
    ...
    return (...args: Args): Promise<Result> => {
      ...
      doneEmitter.on(
        `${job.id}`,
        (data: { $case: 'success'; result: Result } | { $case: 'failure'; result: Error }) => {
